...
import { isValidUrl, isBlankPage, isRestrictedUrl, isAllowedByDomainFilter } from './active-tab-manager';
...
describe('isAllowedByDomainFilter', () => {
	test('returns true when the domain list is empty', () => {
		expect(isAllowedByDomainFilter('https://example.com', [])).toBe(true);
	});

	test('blocks an exact domain match', () => {
		expect(isAllowedByDomainFilter('https://youtube.com/watch?v=123', ['youtube.com'])).toBe(false);
	});

	test('blocks a subdomain of a listed domain', () => {
		expect(isAllowedByDomainFilter('https://www.youtube.com/watch?v=123', ['youtube.com'])).toBe(false);
	});

	test('allows domains that are not listed', () => {
		expect(isAllowedByDomainFilter('https://example.com', ['youtube.com', 'facebook.com'])).toBe(true);
	});

	test('does not match a partial/suffix domain that is not a subdomain', () => {
		expect(isAllowedByDomainFilter('https://notyoutube.com', ['youtube.com'])).toBe(true);
	});

	test('returns false for invalid URLs', () => {
		expect(isAllowedByDomainFilter('not-a-url', ['youtube.com'])).toBe(false);
	});
});
