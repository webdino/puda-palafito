...
import { getMessage, getEffectiveLanguage } from './i18n';
import { updateTokenCount } from './token-counter';
import { summarize, isSummarizerSupported, isSummarizerAvailable } from './summarizer';
...
export async function sendToLLM(promptContext: string, content: string, promptVariables: PromptVariable[], model: ModelConfig): Promise<{ promptResponses: any[] }> {
	...
	try {
		const systemContent =
			`You are a helpful assistant. Please respond with one JSON object named \`prompts_responses\` — no explanatory text before or after. Use the exact keys from the input \`prompts\` object and fill in the values. Values should be Markdown strings unless otherwise specified. Make your responses concise. For example, if the input has keys \`key_a\` and \`key_b\`, your response should look like: {"prompts_responses":{"key_a":"tag1, tag2, tag3","key_b":"- bullet1\n- bullet 2\n- bullet3"}}`;
...
export async function initializeInterpreter(template: Template, variables: { [key: string]: string }, tabId: number, currentUrl: string, onSummary?: (summary: string) => void) {
	...
	const hasSummaryPrompt = !!generalSettings.summaryPrompt?.trim();

	// Hide interpreter if it's disabled or there are no prompt variables and no summary prompt
	if (!generalSettings.interpreterEnabled || (promptVariables.length === 0 && !hasSummaryPrompt)) {
	...
	if (template) {
		...
		if (interpretBtn && !generalSettings.interpreterAutoRun) {
			const clickListener = async () => {
				...
				const result = await handleInterpreterUI(template, variables, tabId, currentUrl, modelConfig);
				if (result.summary !== undefined && onSummary) {
...
export async function handleInterpreterUI(
	...
	try {
		...
		const summaryPrompt = generalSettings.summaryPrompt?.trim();
		if (!summaryPrompt) {
			throw new Error('No summary prompt configured.');
		}
		const promptVariables: PromptVariable[] = [{ key: '__summary__', prompt: summaryPrompt }];
		...
		console.log('Summary generated by LLM:', { provider: provider.name, model: modelConfig.name, modelId: modelConfig.providerModelId });
		...
		const summaryResponse = promptResponses.find(r => r.key === '__summary__');
		return { summary: summaryResponse?.user_response ?? undefined };

	} catch (error) {
		console.error('Error processing LLM:', error);

		// Fall back to Chrome's built-in Summarizer before surfacing an error.
		try {
			if (isSummarizerSupported()) {
				const { code } = await getEffectiveLanguage();
				const textToSummarize = promptContextTextarea.value || variables['{{content}}'] || '';
				const title = variables['{{title}}'] || '';
				const summary = (await isSummarizerAvailable(code))
					? await summarize(title, textToSummarize, code)
					: '';
				if (summary.trim()) {
					console.log('Summary generated by Chrome Summarizer (on-device):', { outputLanguage: code });
			...
			console.error('Summarizer fallback failed:', fallbackErr);
		}
