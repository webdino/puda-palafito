...
import { initializeSummarizerModelSection } from './summarizer-settings';
...
export async function initializeInterpreterSettings(): Promise<void> {
	try {
		...
		try {
			await initializeSummarizerModelSection();
		} catch (error) {
			console.error('Error initializing summarizer model section:', error);
		}
	} catch (error) {
...
function saveInterpreterSettingsFromForm(): void {
	...
	const summaryPromptInput = document.getElementById('summary-prompt') as HTMLTextAreaElement;
	if (summaryPromptInput) {
		updatedSettings.summaryPrompt = summaryPromptInput.value;
	}
