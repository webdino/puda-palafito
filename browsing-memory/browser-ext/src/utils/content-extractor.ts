...
async function sendTimeOnPageRequest(tabId: number): Promise<number> {
	try {
		const response = await browser.runtime.sendMessage({
			action: "sendMessageToTab",
			tabId: tabId,
			message: { action: "getTimeOnPage" }
		}) as { timeOnPage?: number };
		return response?.timeOnPage ?? 0;
	} catch {
		return 0;
	}
}
...
async function sendExtractRequest(tabId: number): Promise<ContentResponse> {
	const [response, timeOnPage] = await Promise.all([
		browser.runtime.sendMessage({
			action: "sendMessageToTab",
			tabId: tabId,
			message: { action: "getPageContent" }
		}) as Promise<ContentResponse & { success?: boolean; error?: string }>,
		sendTimeOnPageRequest(tabId),
	]);
